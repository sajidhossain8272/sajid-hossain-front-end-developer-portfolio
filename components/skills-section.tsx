...
export  function SoftSkillsSection() {
  ...
  return (
    <motion.div
      ...
      className="mt-20"
    >
      <div className="relative backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-7 sm:p-12 shadow-2xl group max-w-3xl mx-auto">
        <motion.h3
          className="text-3xl sm:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-blue-400 to-cyan-400 text-center mb-3 drop-shadow-sm"
          whileInView={{ scale: [0.96, 1] }}
          transition={{ duration: 0.5 }}
        >
          Soft Skills & Expertise
        </motion.h3>
        <p className="text-white/80 text-center mb-7 text-base sm:text-lg font-medium">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pb-1 justify-center">
          {visibleSkills.map((skill, idx) => (
            <motion.span
              key={skill}
              className="
                px-4 py-2 sm:px-6 sm:py-3 bg-gradient-to-r from-white/10 to-white/5
                backdrop-blur-sm border border-white/20 rounded-2xl text-white/90
                font-medium text-sm sm:text-base transition-transform duration-300
                cursor-default min-w-[110px] text-center whitespace-nowrap
            ...
            >
              {skill}
              <span
                className="absolute -inset-1 rounded-2xl bg-gradient-to-r from-cyan-400/10 via-white/5 to-pink-400/10 blur-sm opacity-30 group-hover:opacity-50 pointer-events-none"
                aria-hidden="true"
              />
            </motion.span>
          ))}
        ...
        {skills.length > collapsedCount && (
          <button
            ...
            className="block px-6 py-2 bg-gradient-to-r from-blue-500/20 to-pink-400/20 text-white/80 font-semibold rounded-2xl border border-white/20 mx-auto mt-4 transition hover:scale-105"
